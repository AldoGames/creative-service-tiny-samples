...
{
    public class Separator : IDrawable, IDirtyable
    {
        #region Types
            ...
            using (var area = new EditorGUILayout.HorizontalScope())
            {
                ...
                if (Draggable)
                {
                    using (var scope = new EditorGUILayout.VerticalScope(GUILayout.MaxWidth(3), GUILayout.ExpandHeight(true)))
                    {
                        ...
                        using (new GUIColorScope(m_Drag ? SelectedColor : UnselectedColor))
                        {
                            GUI.DrawTexture(scope.rect, UTinyIcons.SeparatorVertical, ScaleMode.StretchToFill);
                        }
