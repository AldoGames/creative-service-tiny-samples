                ...
                switch (reader.ReadByte())
                {
                    case BinaryToken.Property:
                    {
                        ...
                        if (string.Equals("$TypeId", propertyName))
                        {
                            ...
                            return new BinaryObjectInfo {TypeId = (UTinyTypeId) typeId, Size = (int)size + sizeof(byte) + sizeof(uint)};
                        }
                    ...
                    default:
                        // Return the the start of this object
                        reader.BaseStream.Position = start;
                        return new BinaryObjectInfo {TypeId = UTinyTypeId.Unknown, Size = 0};
                }
            }
        }
